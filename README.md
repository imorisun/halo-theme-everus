...
[![Version](https://img.shields.io/badge/Version-2.2.0-26a760)](https://github.com/imorisun/halo-theme-everus/releases)
...
- 🌗 **暗色 / 亮色双配色** — 后台一键切换：暗色系为默认的深邃黑色基底，亮色系为白色基底 + 绿色点缀的杂志风格；文字对比度均满足 WCAG AA，文章灯箱与音乐播放器在两种配色下保持暗色以保证最佳观感
...
- 🧭 **文章目录（TOC）** — 自动提取文章标题生成目录胶囊条，滚动时高亮当前章节；超宽屏（≥1780px）自动升级为悬浮在正文左侧的目录栏，绝不遮挡正文与评论
- 📝 **文章增强** — 字数统计与阅读时长、上一篇 / 下一篇导航、版权声明（支持 `{title}` / `{url}` / `{author}` 占位）、按标签相关文章推荐、文章点赞
- 📋 **代码块复制按钮** — 文章与页面中的代码块右上角一键复制
- 📈 **阅读进度条** — 页面顶部进度条随滚动增长
- ✨ **粒子背景** — 可选漂浮粒子动画，颜色跟随主题色，尊重系统「减少动态效果」偏好
- 🎨 **主题色自定义** — 一键更换全局强调色，衍生色自动计算
- 🏗️ **代码注入** — 支持自定义 `<head>` 代码、页脚代码与自定义 CSS，可接入统计、客服组件或深度定制样式
- 🕰️ **站点运行时间** — 页脚显示网站已运行时长
- 🖼️ **Banner 文字** — 首页 Banner 可显示自定义主标题与副标题
- 🧩 **首页模块开关** — 最新文章 / 最新动态 / 结语引用均可独立开关，文章数量与结语文案可自定义
- 🔗 **作者页社交链接** — 可视化添加社交链接（支持 Iconify 图标库）
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| 配色方案 | 暗色系 / 亮色系切换；亮色系为白色基底 + 绿色点缀，灯箱与播放器保持暗色 | 暗色系 |
| 主题色 | 全局强调色（链接、按钮、图标等），衍生色自动计算 | `#26a760` |
| 自定义字体 URL | 字体文件直链地址，支持 woff2 / woff / ttf / eot / svg 格式 | 留空，使用系统默认字体 |
| 自定义 CSS | 追加到每个页面 `<head>` 末尾的样式，可覆盖主题默认样式 | 无 |
...
| Banner 主标题 | 显示在 Banner 中央的大标题 | 无 |
| Banner 副标题 | 显示在主标题下方的文字 | 无 |
| 显示最新文章 | 首页「最新文章」板块开关 | 开启 |
| 最新文章数量 | 「最新文章」板块展示的文章条数 | 8 |
| 显示最新动态 | 首页「最新动态」板块开关（需 Moments 插件） | 开启 |
| 最新动态数量 | 「最新动态」板块展示的瞬间条数 | 6 |
| 显示结语引用 | 首页底部结语开关 | 开启 |
| 自定义结语 | 替换默认结语文字 | 留空，使用默认文案 |

### 文章页

| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| 显示文章目录 | 自动提取标题生成胶囊式目录，少于 3 个标题自动隐藏；超宽屏自动升级为正文左侧悬浮栏 | 开启 |
| 显示字数与阅读时长 | 文章标题下方显示字数统计与预计阅读时长 | 开启 |
| 代码块复制按钮 | 文章与页面代码块右上角复制按钮 | 开启 |
| 显示上一篇 / 下一篇 | 文章末尾导航 | 开启 |
| 显示版权声明 | 文章末尾显示版权声明 | 关闭 |
| 自定义版权声明 | 支持 `{title}` / `{url}` / `{author}` 占位符，留空显示默认声明 | 无 |
| 显示相关文章 | 按第一个标签推荐 4 篇相关文章 | 关闭 |
| 显示文章点赞 | 文章末尾显示点赞按钮 | 关闭 |
...
| 社交链接 | 可视化添加社交链接（名称 + 链接 + Iconify 图标），展示在作者页侧边栏 | 无 |
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| 粒子背景 | 页面背景漂浮粒子动画，颜色跟随主题色 | 关闭 |
| 粒子数量 | 粒子数量（10 ~ 150） | 60 |
| 阅读进度条 | 页面顶部显示阅读进度条 | 开启 |
| 显示站点运行时间 | 页脚显示网站已运行时长 | 关闭 |
| 网站成立日期 | 用于计算运行时间，如 `2020-01-01` | 无 |
| 自定义 head 代码 | 注入到页面 `<head>` 末尾的 HTML（统计代码、验证 meta 标签等） | 无 |
| 页脚代码 | 注入到 `</body>` 前的 HTML（统计脚本、客服组件等） | 无 |
...
**Q: 如何切换到亮色系？**
> A: 在 Halo Console → 外观 → EverUs 设置 → 样式与字体 → 配色方案中选择「亮色系」并保存，刷新页面后生效。亮色系下文章灯箱与音乐播放器刻意保持暗色，以保证图片与播放器 UI 的最佳观感。

**Q: 文章目录（TOC）不显示？**
> A: 请确认主题设置 → 文章页中「显示文章目录」已开启。当文章标题（h2 ~ h4）少于 3 个时目录会自动隐藏；目录以胶囊条形式内联在正文上方，仅在超宽屏（≥1780px）下悬浮于正文左侧空白区，不会遮挡正文与评论。

**Q: 文章页的上一篇 / 下一篇、相关文章为什么不显示？**
> A: 上一篇 / 下一篇依赖 `postFinder.cursor`，相关文章依赖文章的第一个标签且需在主题设置 → 文章页中分别开启；站点文章数量过少或无标签时可能没有可展示的内容。

**Q: 如何接入统计代码？**
> A: 在主题设置 → 全局 →「自定义 head 代码」或「页脚代码」中粘贴统计脚本即可，无需修改主题文件。
